...
6. 在“接口参数映射”索引直接浏览全部接口→参数组合；点击接口卡聚焦图谱，点击参数胶囊打开详情。
7. 固件默认居中；组件围绕固件，接口围绕所属组件，参数围绕所属接口。展开哪个节点，下一层就以它为局部中心。
8. 使用滚轮或左上角按钮缩放，点击准星回到固件中心；碰撞层按卡片矩形自动分离。
9. 搜索会把画布自动移到最深层命中节点；展开命中接口后，其直接参数即使不含搜索词也会显示。
10. 使用“重置自动布局”重新计算位置；节点可随时折叠。
11. 需要 UBUS/IPC 等内部调用时进入“高级图谱”或“原始证据”。它们不会混入默认 Web 接口图。
...
| 接口参数映射索引 | 默认显示接口方法、路径、Handler 与参数胶囊；支持折叠、聚焦接口和打开参数详情 | 直接复用 Catalog owner/child identity，不按名称拼接关系 |
...
![AC9 的 28 组接口参数映射索引](./screenshots/2026-08-21-r2-45-interface-parameter-mapping-index.jpg)
...
| Python 全量回归 | `PYTHONPATH=src python3 -m unittest discover -s tests` | 564/564 通过（588.743s） |
| Python 编译检查 | `python3 -m compileall -q src` | 通过 |
| Console 全量回归 | `pnpm test` | 40/40 通过，10 个测试文件 |
...
| 浏览器交互 | 映射索引、父节点局部环绕、搜索聚焦、接口参数展开、参数详情 | AC9 显示 28 组/80 参数；点击 `UploadCfg` 后 4 nodes/3 edges；`filename` 详情通过 |
...
[R2-45 接口参数映射索引记录](./progress/2026-08-21-r2-45-interface-parameter-mapping-index.md)。
