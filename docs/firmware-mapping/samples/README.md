...
- [M1-12 通信架构 research-case corpus](./m1-12-research-case-corpus.json)：保存 AC9 split web stack、X5000R shared-CGI、AC9 DLNA 分层和 FRITZ!Box 4040 Frontend/Native UBUS 分裂的内容寻址证据时间线；保留早期缺口、反事实、义务迁移和静态边界。
...
- [R2-29 AC9 complete historical coverage ledger](./r2-29-vendor-tenda-ac9-historical-coverage-ledger.json)：从真实 AC9 auto-v20 冷启动，将 14 条结构化期望与 57 条开放队列合并为完整 71 条分母；9 observed、2 partial、60 not assessable，并验证 SQLite 幂等发布、HTTP 原因查询和 Console 全分母交互合同。
- [R2-30 OpenWrt AC9 raw artifact analysis](./r2-30-openwrt-ac9-raw-artifact-analysis.json)：原始 `.trx` 经固定镜像隔离解包、保守 rootfs 选择和统一 AnalyzeRun 生成的可回放结果；对应 [图谱](./r2-30-openwrt-ac9-raw-artifact-graph.json) 直接可发布到 Console。
