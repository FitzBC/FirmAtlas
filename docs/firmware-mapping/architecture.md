...
M1-11 Corpus Report Module 的公开 Interface 是 `build_corpus_report(CorpusReportInput) -> CorpusReport`。它只读取不可变 Discovery Catalog 与显式研究样本定义，不重新分析源码，也不从路径风格推断架构类别。Module 分开记录 `real_firmware`、`derived_firmware`、`contract_fixture` 与 `external_lead`；只有 real firmware 的预期制品摘要与 Catalog 一致、coverage completed、所需 Evidence Capability 全部满足、禁止能力未出现且没有开放义务时，类别才能成为 `verified`。v1alpha2 的 `scope_candidate_ids` 允许在混合 Catalog 内验收一个显式候选子集，候选、EvidenceAtom 和候选目标义务使用同一范围；未知或跨 Catalog 范围会拒绝，避免其他架构证据污染类别结论。样本编排脚本是 benchmark Adapter，不属于 Mapper 核心 Interface。
