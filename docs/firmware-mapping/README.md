...
本文不把计划描述成已经实现的能力。状态表中的“完成”必须同时具有实现、回归记录、Git 修订、部署修订和远端验证证据。
...
| --- | --- | --- | --- | --- |
| M0 设计基线 | 进行中 | 理论、领域、架构、集成、评测和协作设计 | 文档互链、领域一致性、仓库验证、部署核验 | [M0 记录](./progress/2026-08-08-m0-design-baseline.md) |
...
**下一项建议**：M1-01。先确定快照外部 Interface 和最小标注集，不先迁移旧工具的 Fusion 或中间 JSON。
