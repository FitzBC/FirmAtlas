...
> 文档 ID：FM-MASTER
> 当前阶段：M1 冷启动发现
> 当前状态：M1-06B 文本后端 Producer 已验证；下一项为 M1-07 线索调度与固定点终止
> 最近更新：2026-08-09
...
| M1-07 | 线索调度与固定点终止 | 未开始 | M1-04/05/06A/06B/06C | determinism/budget tests |
...
**下一项建议**：实现 M1-07 线索调度与固定点终止，把各 Producer 的 coverage、candidate association 与 unresolved obligation 合并为确定性、预算受控的工作队列；随后进入 M1-08 无 seed 候选目录。Native Deep Adapter 的 anchor/xref 请求继续以 M1-06C obligation 为输入。M1-02B 可使用 Binwalk，但必须在生产隔离 worker 和固定工具链中执行，不能进入主分析进程。
