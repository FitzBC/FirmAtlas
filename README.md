...
固件资产页聚合公开 benchmark、厂商下载中心、研究数据集和社区归档。首批同步会登记 18 个来源，从 FirmEmuHub 收录 100 个固件候选，并根据 IoTVulBench 建立 95 条漏洞复现环境线索（覆盖 15 个不同固件）。同步过程只读取文本元数据，不下载固件二进制，也不会把候选地址冒充为已经校验的制品。

```bash
# 把公开来源、样本候选和漏洞关系写入 var/firmatlas.db；可安全重跑
...
UI 和 API 会明确区分“候选地址”“漏洞复现线索”和“已下载/哈希校验制品”。完整来源清单、15 个已验证可访问的 raw 地址、SHA-256、95 个 CVE 分组及数据质量异常见[固件样本来源研究](./docs/research-firmware-sample-sources.md)。
