...
| 动态接口力导图 | 固件默认居中，每一级子节点围绕实际展开的父节点形成局部簇；四类对象使用不同轮廓与色带 | 父节点局部环、画布平移、滚轮/按钮缩放、回到固件中心、矩形零重叠；只有存在子参数的接口显示展开箭头 |
...
邻接关系，滚轮缩放；固件默认位于画布中心，组件围绕固件，接口围绕所属组件，参数围绕所属接口，
展开哪个节点就以哪个节点为局部布局中心，不再把所有类别投影为固件中心的全局对称环。空白画布可按住拖动平移，
左上角提供缩放和“回到固件中心”。固件使用绿色双边框，组件使用青色实线侧边条，接口使用紫色
虚线卡片，参数使用琥珀色胶囊。搜索只保留命中分支及其祖先，自动布局可随时重置。点击节点主体或箭头都会
...
搜索词。图谱上方默认显示可折叠的“接口参数映射”索引，逐组列出方法、接口、Handler、参数位置和
确定性类型；点击映射聚焦接口，点击参数胶囊打开详情。参数继续围绕所属接口形成局部簇。
`ubus://`、IPC 等内部逻辑操作不会作为 Web URL 混入默认接口图，需要时可从
