...
| 动态接口力导图 | 以固件为根，只展示真实二进制 → Web 接口 → 参数；点击节点主体逐层展开，拖拽只移动；支持矩形碰撞分离、悬停邻接高亮、滚轮缩放、搜索与折叠 | 大图首屏固定保留固件/组件层；只有存在子参数的接口显示展开箭头；静态前端资源仅保留为证据 locator |
...
邻接关系，滚轮缩放；搜索只保留命中分支及其祖先，自动布局可随时重置。点击节点主体或箭头都会
