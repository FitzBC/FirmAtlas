...
该入口会自动建立 Source Plan，运行 Frontend、跨资源 Frontend Asset Graph、Frontend Feature Gate、Frontend Invocation Reachability、Web configuration、Script backend、Native shallow、Correlation 和 Scheduler，并按版本化 Profile/Registry 自动选择适用的 ARM PIC、Native ubus 等确定性深化 Adapter，最后发布不可变 Discovery Catalog；单个 producer 失败会保留为 partial stage/coverage，不会伪装成空成功。可选 `--graph-output` 会从同一 Catalog 生成确定性的通信架构图 read model；`--graph-focus` 可重复，按精确接口身份进行有界语义跳转，制品归属边不会扩大焦点。当前默认冻结为 `auto-v18`：它继承 `auto-v17` 的主配置文本导入，并把 `/webroot/default_url.cfg` 恢复为独立 latent consumer：`load_url_mib@0x8d0c → parser@0x766c → hash_insert → cfm/url_mib/*`。AC9 rootfs 没有静态第二文档，且全 ELF 静态扫描没有找到 `cfm Upload/RestoreMTD → load_url_mib/reload_url_mib` 激活边，因此 Catalog 只发布 candidate 状态范围、0 个已声明键和开放的激活义务；不会把主 `default.cfg` 的 1013 个唯一键复制进 URL 域。主链仍从 `POST /cgi-bin/UploadCfg → httpd → libtpi:tpi_sys_cfg_upload → RestoreMTD → InitDefaultCfm → load_mib` 恢复 `security.ddos.map` 与 `sys.schedulereboot.*` 等带行证据的 configuration-state，而不是 HTTP 参数。`auto-v16` 与 `auto-v17` 均保留冻结回放。图谱本身仍不是推理器，也不会创造运行时或漏洞事实。首要样本最新结果见 [R2-26 URL 配置文档消费者](./docs/firmware-mapping/progress/2026-08-13-r2-26-ac9-configuration-url-document.md)、[R2-25 配置文本导入修正](./docs/firmware-mapping/progress/2026-08-12-r2-25-ac9-configuration-text-import.md)与 [R2-24 历史 IPC 解释](./docs/firmware-mapping/progress/2026-08-12-r2-24-ac9-configuration-blob-state-flow.md)。参数线索索引对已验证的前端请求参数执行有界同固件精确 token 检索，显式发布阳性、阴性与覆盖受限结果，但不会把字符串共现冒充数据流。
