...
| 动态接口力导图 | 固件默认居中，组件、接口和参数按层级环绕；四类对象使用不同轮廓与色带；节点和空白画布可分别拖动 | 同心环力布局、画布平移、滚轮/按钮缩放、回到固件中心、矩形零重叠；只有存在子参数的接口显示展开箭头 |
...
邻接关系，滚轮缩放；固件默认位于画布中心，组件、接口与参数在外层环绕，空白画布可按住拖动平移，
