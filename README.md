...
该入口会自动建立 Source Plan，运行 Frontend、跨资源 Frontend Asset Graph、Frontend Feature Gate、Frontend Invocation Reachability、Web configuration、Script backend、Native shallow、Correlation 和 Scheduler，并按版本化 Profile/Registry 自动选择适用的 ARM PIC、Native ubus 等确定性深化 Adapter，最后发布不可变 Discovery Catalog；单个 producer 失败会保留为 partial stage/coverage，不会伪装成空成功。可选 `--graph-output` 会从同一 Catalog 生成确定性的通信架构图 read model；`--graph-focus` 可重复，按精确接口身份进行有界语义跳转，制品归属边不会扩大焦点。当前默认冻结为 `auto-v17`：它保留 `auto-v15` 的 symbol-sized command table 与 ARM `BL → PLT relocation → DT_NEEDED/export` 跨 ELF 调用链，并以配置文本导入分析替代 `auto-v16` 的 provisional configuration-image interpretation。AC9 现从 `POST /cgi-bin/UploadCfg → httpd → libtpi:tpi_sys_cfg_upload` 恢复分隔符、`/webroot/default.cfg`、`/webroot/default_url.cfg` 与 `cfm Upload`，再从 `RestoreMTD → InitDefaultCfm → load_mib → strtok/strchr/strdup/hash_insert` 恢复 `cfm/default_mib/*` 的逐键导入。默认文档产生 1015 条有序声明、1013 个唯一配置键；`security.ddos.map` 与 `sys.schedulereboot.*` 在图谱中是带可重放行证据的 configuration-state 节点和 `imports_state` 边，不是 HTTP 参数。`auto-v16` 仍可精确重放 R2-24 当时的 IPC 发现，但其 `configuration_partition[0]` whole-image 解释已被实现层证据否定，不进入当前图谱。图谱本身仍不是推理器，也不会创造运行时或漏洞事实。首要样本最新结果见 [R2-25 配置文本导入修正](./docs/firmware-mapping/progress/2026-08-12-r2-25-ac9-configuration-text-import.md)、[R2-24 历史 IPC 解释](./docs/firmware-mapping/progress/2026-08-12-r2-24-ac9-configuration-blob-state-flow.md)与 [R2-23 跨 ELF 配置链](./docs/firmware-mapping/progress/2026-08-12-r2-23-ac9-cross-elf-persistence.md)。参数线索索引对已验证的前端请求参数执行有界同固件精确 token 检索，显式发布阳性、阴性与覆盖受限结果，但不会把字符串共现冒充数据流。
