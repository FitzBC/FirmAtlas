...
| 状态 | 模块 |
| --- | --- |
| **Available** | 漏洞情报同步与检索、固件候选目录、版本/CPE 关联、双向下钻、接口与参数语义分析、架构风格分类与推荐、测绘 Snapshot v1alpha1 合同验证器与证据回放示例 |
| **Next** | 无样例固件清单与线索发现、固件上传与 SHA-256 制品去重、隔离递归解包、文件系统与组件 SBOM、服务与监听端口测绘 |
