...
复杂通信结构现在会进入内容寻址的研究案例库，而不是只留在阶段性说明中。AC9 案例保存了 `前端 /goform → nginx namespace 不相交 → ownership obligation → httpd/dhttpd shallow 对照 → ARM PIC call-site`；X5000R 案例保存了 `共享 cstecgi.cgi → config.js/topicurl.js 跨资源绑定 → 199 个 operation → 四张 MIPS inline table → 123/199 handler binding → 76/14 前后端差集`。Frontend Asset Graph 保留定义端/消费端两处 EvidenceAtom，Native Profile 则要求 route、动态表符号、完整 entry 和 executable handler 四线证据；`type:this.type` 与 handler 下游 value-flow 继续标为未决。详见[研究案例库](./docs/firmware-mapping/research-casebook.md)、[机器可读案例](./docs/firmware-mapping/samples/m1-12-research-case-corpus.json)、[Frontend 中间输出](./docs/firmware-mapping/samples/m1-15-x5000r-frontend-asset-graph.json)与[MIPS dispatcher 中间输出](./docs/firmware-mapping/samples/m1-16-x5000r-mips-dispatch.json)。
