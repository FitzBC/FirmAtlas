...
该入口会自动建立 Source Plan，运行 Frontend、跨资源 Frontend Asset Graph、Web configuration、Script backend、Native shallow、Correlation 和 Scheduler，并按版本化 Profile/Registry 自动选择适用的 ARM PIC、Native ubus 等确定性深化 Adapter，最后发布不可变 Discovery Catalog；单个 producer 失败会保留为 partial stage/coverage，不会伪装成空成功。当前默认 `auto-v10` 在 `auto-v9` 基线上识别 JavaScript regex literal，避免正则中的引号吞掉后续请求；Native ARM v4 则恢复“route/handler 装载后跳入共享 registrar tail”的非连续注册形态，并只对这类已验证前端 handler 自动执行函数级 PIC/GOT literal-xref 深化。AC9 因此恢复 `main.js → GetUSBStatus → formGetUSBStatus@0xa62d0 → dlna.en / /var/etc/upan / dlna`；这是一条 USB 状态路径，不会被冒充为四条仍未绑定的 DLNA 配置操作。`auto-v9` 及更早 Profile 继续冻结重放。首要样本最新结果见 [R2-13 原厂 Tenda AC9](./docs/firmware-mapping/progress/2026-08-10-r2-13-ac9-tail-merged-usb-status.md)，上一冻结基线见 [R2-12](./docs/firmware-mapping/progress/2026-08-10-r2-12-ac9-daemon-command-chain.md)，同硬件 OpenWrt 对照见 [R2-02 OpenWrt AC9](./docs/firmware-mapping/samples/r2-02-tenda-ac9-auto-profile.json)。参数线索索引对已验证的前端请求参数执行有界同固件精确 token 检索，显式发布阳性、阴性与覆盖受限结果，但不会把字符串共现冒充数据流。
