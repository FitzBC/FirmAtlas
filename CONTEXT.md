...
**配置镜像状态流（Configuration Image State Flow）**：
由已验证的消息 framing、操作码分发、payload 解码与整镜像恢复 writer 共同证明的配置镜像到状态范围关系。它必须保留消息大小、payload 偏移和状态粒度；若未证明逐键 parser，只能发布分区或 wildcard 状态范围，不能把内部配置键写成接口参数。
_Avoid_: HTTP 参数、逐键参数—状态映射、运行时已执行、漏洞数据流
